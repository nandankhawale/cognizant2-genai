...
const Chatbot = ({ onClose }) => {
  ...
  const loanTypes = [
    { id: 'education', text: 'Education Loan', description: 'For higher education and courses' },
    { id: 'home', text: 'Home Loan', description: 'For purchasing or constructing property' },
    { id: 'personal', text: 'Personal Loan', description: 'For personal expenses' },
    { id: 'gold', text: 'Gold Loan', description: 'Quick loan against your gold jewelry' },
    { id: 'business', text: 'Business Loan', description: 'For business expansion and working capital' }
  ];
  ...
  const quickActions = [
    ...
    { id: 6, text: 'Start Over', action: 'restart' }
  ];
  ...
  const startLoanApplication = async (loanType) => {
    setIsLoading(true);
    const loadingId = addMessage('Starting your loan application...', 'bot', true);
    
    try {
      const response = await axios.post(`${API_BASE_URL}${API_CONFIG.ENDPOINTS.CHAT_START}`, {
        loan_type: loanType
      });
      ...
      updateMessage(loadingId, 'Sorry, I encountered an error starting your application. Please try again.');
  ...
  const sendMessageToAPI = async (message) => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}${API_CONFIG.ENDPOINTS.CHAT_MESSAGE}`, {
      ...
      if (response.data.prediction) {
        ...
        const loanTypeName = currentLoanType.charAt(0).toUpperCase() + currentLoanType.slice(1);
...
✅ **YES! You are eligible for ₹${requestedAmount.toLocaleString()} at ${result.interest_rate}% per annum**
...
✅ **You can get up to ₹${eligibleAmount.toLocaleString()} at ${result.interest_rate}% per annum**
    ...
    if (inputMessage.trim() && !isLoading) {
      ...
      if (sessionId) {
        ...
        if (lowerMessage.includes('education')) {
          ...
          setTimeout(() => {
            addMessage('Please select a loan type from the options below, or tell me which type of loan you need (education, home, or personal).', 'bot');
          }, 500);
        }
      }
          ...
          {messages.map((message) => (
            ...
            >
              ...
              >
                ...
                <p className={`text-xs mt-1 ${message.sender === 'bot' ? 'text-gray-500' : 'text-blue-200'}`}>
      ...
      <div className="px-4 py-3 bg-white border-t border-gray-200">
        <div className="flex items-center space-x-3">
          <input
            ...
            placeholder={sessionId ? "Tell me about your loan requirements..." : "Type your message or select a loan type..."}
            disabled={isLoading}
